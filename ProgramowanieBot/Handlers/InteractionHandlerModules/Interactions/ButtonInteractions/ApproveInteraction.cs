...
public class ApproveInteraction(IServiceProvider serviceProvider, ConfigService config) : InteractionModule<ButtonInteractionContext>
{
    ...
    public async Task ApproveAsync(ulong helper, bool giveReputation, ulong? helper2 = null, bool? giveReputation2 = null)
    {
        ...
        {
            m.Content = $"**{config.Emojis.Success} {config.Interaction.PostResolvedResponse}**";
            m.Components = [];
        }));
        ...
        await channel.ModifyAsync(t =>
        {
            ...
            var name = $"{config.Interaction.PostResolvedPrefix} {channel.Name}";
